...
describe("most blogs", () => {
  test("return the author with most blogs with the number of blogs", () => {
    const result = mostBlogs(biggerList);

    expect(result) == { author: "Jane Smith", blogs: 1 };
  });
});

describe("most likes", () => {
  test("return the author with most likes with the total number of likes", () => {
    const result = mostLikes(biggerList);

    expect(result) == { author: "Sam Carter", likes: 550 };
  });
});
